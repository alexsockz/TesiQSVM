...
adhoc_feature_map = ZZFeatureMap(feature_dimension=dimensions, reps=2, entanglement=ent[0], insert_barriers=True)
trans_feature_map = pm.run(adhoc_feature_map)

adhoc_ansatz = RealAmplitudes(num_qubits=dimensions, entanglement=ent[0], reps=repetitions, insert_barriers=True)
trans_ansatz = pm.run(adhoc_ansatz)

trans_ansatz.draw(output="mpl",style="clifford")
plt.show()
